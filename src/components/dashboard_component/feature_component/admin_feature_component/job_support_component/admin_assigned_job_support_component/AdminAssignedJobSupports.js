...
function AdminAssignedJobSupports() {
    ...
    const handlePreviousNext = async (event) => {
        ...
        //await getHoldAssignedJobSupportingsOnPage();
    ...
    const assignedJobSupportWithJobSupporterDisplayErrMsg = (errorMessage) => {
    ...
    const assignedJobSupportWithJobSupporterDisplaySucMsg = (errorMessage) => {
    ...
    const [assignedJobSupports, setAssignedJobSupports] = useState([]);
    const getHoldAssignedJobSupportingsOnPage = async () => {
        setLoadingBar(true);
        const request = {
            page: page,
            size: size,
        ...
        try {
            const responseData = await AdminService.getHoldAssignedJobSupportingsOnPage(request);
            console.log(responseData);

            setIsLastPage(responseData.isLastPage);
            setAssignedJobSupports(responseData.list);
            if (responseData.list.length === 0) {
                assignedJobSupportWithJobSupporterDisplayErrMsg('Assigned Job Supports are not found');
            }
        } catch (error) {
            console.log(error.message);
            handleGetHoldAssignedJobSupportWithJobSupporterErrors(error.message);
        } finally {
            setLoadingBar(false);
        }
    }

    const handleGetHoldAssignedJobSupportWithJobSupporterErrors = (errorStatus) => {
        if (Constants.BREEZE_DATABASE_EXCEPTION === errorStatus)
            assignedJobSupportWithJobSupporterDisplayErrMsg("Sorry, Our service is down");
        else
            assignedJobSupportWithJobSupporterDisplayErrMsg("Could not process your request");
    }

    useEffect(() => {
        getHoldAssignedJobSupportingsOnPage();
    ...
    const handleViewHoldAssignedJobSupporting = async (jobSupportBookingId) => {
        setLoadingBar(true);
        ...
        try {
            const responseData = await AdminService.viewHoldAssignedJobSupport(request);
            console.log(responseData);
    ...
    const handleViewHoldJobSupportingErrors = (errorStatus, jobSupportBookingId) => {
        if (Constants.INVALID_REQUEST_FIELD === errorStatus)
            handleViewHoldAssignedJobSupportingErrMsg("Job Support Booking id is invalid", jobSupportBookingId);
        else if (Constants.ENTITY_NOT_FOUND === errorStatus)
            handleViewHoldAssignedJobSupportingErrMsg("Job Support Booking is Not Found", jobSupportBookingId);
        else if (Constants.BREEZE_DATABASE_EXCEPTION === errorStatus)
            handleViewHoldAssignedJobSupportingErrMsg("Sorry, Our service is down", jobSupportBookingId);
        else
            handleViewHoldAssignedJobSupportingErrMsg("Could not process your request", jobSupportBookingId);
    }

    const handleViewHoldAssignedJobSupportingErrMsg = (errorMessage, jobSupportBookingId) => {
    ...
    const handleChangeAdminStatus = async (assignedStatus, jobSupportBookingId) => {
        ...
        setLoadingBar(true);
        try {
            const responseData = await AdminService.confirmOrWithdrawAssignedJobSupport(changeAdminStatusRequest);
            console.log(responseData);
    ...
    return (
        <div className='admin-assigned-job-supports' style={{ fontSize: '14px' }}>
            {loadingBar && <LoadingBar />}
            <div className='mt-1' style={{ display: 'flex', flexWrap: 'wrap', maxHeight: '80vh', overflow: 'auto' }}>
                {assignedJobSupports.map((item, index) => (
                                ...
                                <div className='job-support'>
                                    <div className='flex'>
                                        ...
                                        <button
                                            ...
                                            onClick={() => handleViewHoldAssignedJobSupporting(item.jobSupportBookingId)}
                                        >
                                            View
                                        </button>
                                    </div>
                    ...
                    <Container className='px-5'>
                        ...
                        <Modal.Body style={{ fontSize: '13px' }}>
                            <div className='job-support-and-job-supporter-data'>
                                ...
                                <div className='assigned-job-supporter-data mt-2'>
                                    <label style={{ textDecoration: 'underline', fontSize: '16px' }}>Assigned Job Supporter Data</label> <br />
                                    <label>Assigned Job Supporter-Id : </label> <span>{viewData.assignedJobSupporter.userId}</span> <br />
                                    <label>Assigned Job Supporter-Name : </label> <span>{viewData.assignedJobSupporter.userFirstname + " " + viewData.assignedJobSupporter.userLastname}</span> <br />
                                    <label>Assigned Job Supporter-Experience : </label> <span>{viewData.assignedJobSupporter.userExperience}</span> <br />
                                    <label>Assigned Job Supporter-Mail : </label> <span>{viewData.assignedJobSupporter.username}</span> <br />
                                    <label>Assigned Job Supporter-Phone : </label> <span>{viewData.assignedJobSupporter.phoneNumberWithCountryCode}</span>
                                </div>
                                ...
                                <div className='job-support-booking-data mt-2'>
                                    ...
                                    <label>Assigned Technical-Stack : </label> <span>{viewData.assignedJobSupportDto.technologyList.map(tech => tech.technologyName).join(', ')}</span> <br />
                                    <div className='time-slots'>
                                        <label>Assigned Booked Slots : </label> 
                                        <div className='time-slot-container'>
                                            {viewData.jobSupportBookingDto.assignedTimeSlotList.map((item, index) => (
                                                <button
                                                    key={index}
                                                    type='button'
                                                    className='time-slot-button'
...
export default AdminAssignedJobSupports;