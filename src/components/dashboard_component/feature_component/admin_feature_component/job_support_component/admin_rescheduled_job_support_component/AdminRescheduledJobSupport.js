...
function AdminRescheduledJobSupport() {
    ...
    const handlePreviousNext = async (event) => {
        ...
        //await getHoldRescheduledJobSupportingsOnPage();
    ...
    const rescheduledJobSupportWithJobSupporterDisplayErrMsg = (errorMessage) => {
    ...
    const rescheduledJobSupportWithJobSupporterDisplaySucMsg = (errorMessage) => {
    ...
    const [rescheduledJobSupportWithJobSupporter, setRescheduledJobSupportWithJobSupporter] = useState([]);
    const getHoldRescheduledJobSupportingsOnPage = async () => {
        setLoadingBar(true);
        const request = {
            page: page,
            size: size,
        ...
        try {
            const responseData = await AdminService.getHoldRescheduledJobSupportingsOnPage(request);
            console.log(responseData);

            setIsLastPage(responseData.isLastPage);
            setRescheduledJobSupportWithJobSupporter(responseData.list);
            if (responseData.list.length === 0) {
                rescheduledJobSupportWithJobSupporterDisplayErrMsg('Rescheduled Job Supports are not found');
            }
        } catch (error) {
            console.log(error.message);
            handleGetHoldRescheduledJobSupportWithJobSupporterErrors(error.message);
        } finally {
            setLoadingBar(false);
        }
    }

    const handleGetHoldRescheduledJobSupportWithJobSupporterErrors = (errorStatus) => {
        if (Constants.BREEZE_DATABASE_EXCEPTION === errorStatus)
            rescheduledJobSupportWithJobSupporterDisplayErrMsg("Sorry, Our service is down");
        else
            rescheduledJobSupportWithJobSupporterDisplayErrMsg("Could not process your request");
    }

    useEffect(() => {
        getHoldRescheduledJobSupportingsOnPage();
    ...
    const handleViewHoldRescheduledJobSupporting = async (jobSupportBookingId) => {
        setLoadingBar(true);
        ...
        try {
            const responseData = await AdminService.viewHoldRescheduledJobSupport(request);
            console.log(responseData);
    ...
    const handleViewHoldJobSupportingErrors = (errorStatus, jobSupportBookingId) => {
        if (Constants.INVALID_REQUEST_FIELD === errorStatus)
            handleViewHoldRescheduledJobSupportingErrMsg("Job Support Booking id is invalid", jobSupportBookingId);
        else if (Constants.ENTITY_NOT_FOUND === errorStatus)
            handleViewHoldRescheduledJobSupportingErrMsg("Job Support Booking is Not Found", jobSupportBookingId);
        else if (Constants.BREEZE_DATABASE_EXCEPTION === errorStatus)
            handleViewHoldRescheduledJobSupportingErrMsg("Sorry, Our service is down", jobSupportBookingId);
        else
            handleViewHoldRescheduledJobSupportingErrMsg("Could not process your request", jobSupportBookingId);
    }

    const handleViewHoldRescheduledJobSupportingErrMsg = (errorMessage, jobSupportBookingId) => {
    ...
    const handleChangeAdminStatus = async (rescheduledStatus, jobSupportBookingId) => {
        ...
        setLoadingBar(true);
        try {
            const responseData = await AdminService.confirmOrWithdrawRescheduledJobSupport(changeAdminStatusRequest);
            console.log(responseData);
    ...
    return (
        <div className='admin-rescheduled-job-supports' style={{ fontSize: '14px' }}>
            {loadingBar && <LoadingBar />}
            <div className='mt-1' style={{ display: 'flex', flexWrap: 'wrap', maxHeight: '80vh', overflow: 'auto' }}>
                {rescheduledJobSupportWithJobSupporter.map((item, index) => (
                                ...
                                <div className='job-support'>
                                    <div className='flex'>
                                        ...
                                        <button
                                            ...
                                            onClick={() => handleViewHoldRescheduledJobSupporting(item.jobSupportBookingId)}
                                        >
                                            View
                                        </button>
                                    </div>
            ...
            {rescheduledJobSupportWithJobSupporter.length !== 0 && (
            ...
            {showModal && (
                <Modal className='view-hold-rescheduled-job-support-modal' size='lg' show={showModal} onHide={handleModalClose} centered backdrop="static">
                    <Container className='px-5'>
                        <Modal.Header closeButton>
                            <Modal.Title style={{ fontSize: '18px' }}>
                                Job Support Booking & Reschedule Data
                            </Modal.Title>
                        </Modal.Header>
                        <Modal.Body style={{ fontSize: '13px' }}>
                            <div className='job-support-and-job-supporter-data'>
                                ...
                                <div className='job-support-rescheduled-data mt-2'>
                                    <label style={{ textDecoration: 'underline', fontSize: '16px' }}>Rescheduled Data</label> <br />
                                    <label>Rescheduled Start Date : </label> <span>{(Constants.convertUserTimezoneDateTime(viewData.jobSupportBookingDto.rescheduledStartDate)).date}</span> <br />
                                    <label>Rescheduled End Date : </label> <span>{(Constants.convertUserTimezoneDateTime(viewData.jobSupportBookingDto.rescheduledEndDate)).date}</span> <br />
                                    <div className='time-slots'>
                                        <label>Rescheduled Slots : </label> 
                                        <div className='time-slot-container'>
                                            {viewData.jobSupportBookingDto.rescheduledTimeSlotList.map((item, index) => (
                                                <button
                                                    key={index}
                                                    type='button'
                                                    className='time-slot-button'
...
export default AdminRescheduledJobSupport; 